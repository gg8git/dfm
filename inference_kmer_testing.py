...
for seqs, _ in train_loader:
    ...
    print(batch_acc_linear, '/', len(stacked_seqs))
    print(batch_acc_ode, '/', len(stacked_seqs))
    ...
    for x0 in stacked_x0s:
        ...
        # pred seq
        seq_linear = norm_logits_linear.argmax(dim=-1)
        seq_ode = norm_logits_ode.argmax(dim=-1)

        pred_acc_smt = torch.sum(seq_linear.to(device) == seq_ode.to(device)) / seq_ode.numel()
        batch_acc_smt += pred_acc_smt
        ...
        pred_linear = norm_logits_linear.argmax(dim=-1)
        pred_ode = norm_logits_ode.argmax(dim=-1)
        pred_acc_linear = torch.sum(pred_linear.to(device) == seq_ode.to(device)) / seq_ode.numel()
    ...
    print(batch_acc_linear, '/', len(stacked_x0s))
    print(batch_acc_ode, '/', len(stacked_x0s))
    print(batch_acc_smt, '/', len(stacked_x0s))
