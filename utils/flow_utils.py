...
class DirichletConditionalFlow:
    ...
    def c_factor(self, bs, alpha):
        bs = np.clip(bs, 1e-8, 1 - 1e-8)
        out1 = scipy.special.beta(alpha, self.K - 1)

        denom = np.power(1 - bs, self.K - 1)
        denom = np.clip(denom, 1e-8, 1e8)
        out2 = np.where(bs < 1, out1 / denom, 0)

        # pow = np.clip(alpha - 1, 1e-8, 1e8)
        denom = np.power(bs, alpha - 1)
        denom = np.clip(denom, 1e-8, 1e8)
        mask = denom > 0
        out = np.zeros_like(out2)
        out[mask] = out2[mask] / denom[mask]
